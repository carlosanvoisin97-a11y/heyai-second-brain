...
- [ ] **#p1 Carlo**: bozza email recap a David Fasulo + Lorena Cavecchi + Arianna Giordanella (CC Federico + Simone) con punti operativi finali + setup IT — bozza in [[50 - Archive/Inbox/2026-05/2026-05-12 - BOZZA Email Recap Crowd Sessione Finale]]
...
- 2026-05-22 — `local_04146f40` — Crowd platform email response: risposta Lorena su thread "R: Crowd | Sessione Finale Test" — video tutorial spinto sul team di sviluppo (linguaggio §13 mai citare Soolutions per nome) + call formazione reparto **spostata a valle dei test** (coerente con stance "chiusura progetto entro fine mese, poi affianchiamo"). Carlo non si impegna sul timing call. Email paste-ready Outlook (TO Lorena+Arianna+David, CC Federico+Simone). 2 nuovi task: #p1 Federico video tutorial, #p2 Carlo call formazione reparto post-test. → [[80 - Sources/Cowork Sessions/2026-05-24 - Indice sessioni#Sessione 6 — `local_04146f40-cd34-471d-98e6-4551e233ff33` — Crowd platform email response]]
...
- [ ] **#p1 Federico**: modalità **evento su invito / lista chiusa** (segnalata Arianna 29/5) — (a) il form pubblico NON deve creare un nuovo record se un invitato modifica i campi identificativi (oggi mail = chiave singola → doppione); niente aggiunta libera di persone fuori lista oltre agli accompagnatori; (b) campi identificativi (nome/cognome/email, opz. matricole) **precompilati e bloccabili in modifica**, configurabili per singolo evento. Il punto (b) risolve di fatto il punto (a). Collegato al task "chiave univoca composita nel re-import"
...
- [ ] **#p1** Federico: guida utente Crowd / video tutorial in sostituzione delle demo registrate non fatte — **anticipata da Lorena 21/5** (le serve per accelerare i test in corso, non più solo "fiocchetto finale"). Carlo ha ri-pushato 22/5: serve riscontro sui tempi per aggiornare Lorena
- [ ] **#p2** Carlo: pianificare call di formazione al reparto Lorena a chiusura progetto (richiesta esplicita Lorena 21/5: *"quando chiuso tutto potrebbe anche essere utile rifare una call di formazione anche alle altre persone di reparto"*)
...
- **Stance tempistiche (decisione)**: linea morbida. NESSUNA scadenza dura comunicata, NESSUN accenno a CR a tariffario — i due punti sono assorbiti nel rilascio finale in corso (coerente col pivot Federico 27/5: completare la delivery finale e dare altri giorni di test). Formula non vincolante usata: *"appena disponibile vi diamo aggiornamenti"* (no promessa di finestra strutturata) + *"il team resta a disposizione per collegarsi live"*.
- **Richiesta operativa ad Arianna/NoLoop**: girare il **report completo di tutti i test** fatti → poi allineamento col team di sviluppo per il **fine tuning finale** → a seguire **ultima call ufficiale di confronto e demo**. Sequenza di chiusura formalizzata verso il cliente. Chiesto anche un esempio concreto del doppione (evento + ospite) per repro.
- ⚠️ **Da triagiare con Carlo**: questa mail è uscita sulla linea tecnica/morbida. Verificare se la sequenza del piano 20/5 — *"[ ] #p1 Carlo: chiamare Lorena al telefono"* + *"[ ] #p1 Carlo: rispondere all'email feedback con scadenza 27/5 + CR a tariffario post-deadline"* — sia ancora prevista o superata dal pivot Federico. Checkbox NON chiusi (la comunicazione scadenza dura non è stata fatta e non risulta eseguito il gate-call Lorena). La modalità invito/lista chiusa resta task aperto lato Federico (delivery finale).

### 2026-05-29 — Segnalazione Arianna (test in corso): enforcement invito/lista chiusa + campi non modificabili
- **Contesto**: Arianna Giordanella continua i test (deadline 27/5 superata, ciclo prolungato come da strategia Federico nota 27/5). Email con due punti operativi.
- **Punto 1 — doppione su evento su invito**: inviando la mail-test a una persona **già in lista ospiti**, se in fase di compilazione del form **modifica nome o email**, il sistema **crea una seconda scheda** e la aggiunge come confermata. Arianna: *"se l'evento è su invito, non dovrebbe permettermi di aggiungere altri se non le persone in lista… la stragrande maggioranza degli eventi che gestiamo sono su invito e lista chiusa… che agli ospiti sia permesso di aggiungere altri (che non siano accompagnatori) liberamente è una possibilità che non devono avere"*.
- **Punto 2 — campi precompilati non modificabili**: chiede se l'invitato possa vedere alcuni campi **precompilati e non editabili** (tipicamente nome, cognome, email; talvolta matricole aziendali e altri dettagli che servono ai clienti), **configurabile per singolo evento**. Teme di aver perso un passaggio tra le call — in realtà è una funzione discussa ma non ancora rilasciata.
- **Diagnosi tecnica**: il match ospite oggi usa la **mail come chiave singola** → modificare email (o nome, se il record è stato risolto su nome) rompe l'associazione all'invitato e genera nuovo record. **Il punto 2 risolve il punto 1**: bloccando in modifica i campi identificativi (nome/cognome/email), l'ospite non può alterarli e non si genera più il doppione. Collegato ai task esistenti: chiave univoca composita nel re-import (sezione "Task aperti" enhancement Wave 1) + god-mode segreteria (lato opposto: bypass controllato per super-admin).
- **Modalità mancante**: una vera modalità **"evento su invito / lista chiusa"** che impedisca la creazione di nuove schede dal form pubblico (oggi il form ne crea sempre una). Da girare a Federico come parte della delivery finale in corso.
- **Contesto**: giorno della deadline test concordata (27/5). Federico, via note vocali WhatsApp, elabora la "delivery finale" che sta completando e la strategia per chiudere il ciclo di test. Fonte: [[2026-05-28 - Trascrizioni vocali WhatsApp (batch 25-28 mag)]] note 03-04.
- **Feature in lavorazione (parte della delivery finale)**: workflow import liste mail/contatti ospiti → l'admin aggiunge **nuovi campi custom** → gli ospiti li **arricchiscono in pagina** (se nome/cognome già presenti nell'import, non vengono richiesti di nuovo). **In lavorazione**: l'enforcement dei **campi obbligatori non saltabili** nella pagina di compilazione. Federico: *"manca ancora… la parte che consente di vedere i campi ma fare in modo che non si possono evitare nella pagina… sono queste piccole cose qua su cui sto lavorando"*. (Lato opposto del god-mode segreteria già a task, che invece consente il salvataggio bypassando i campi obbligatori.)
- **Strategia chiusura test (richiesta Federico)**: NON forzare la chiusura del ciclo di test finché lui non rilascia la **delivery finale**; meglio concedere agli utenti **più giorni per testare DOPO** la delivery, per non ricevere segnalazioni su feature non ancora terminate / integrazioni mancanti (tempo perso a rispondere su non-bug). Si offre di collegarsi **live** con gli utenti per aiutarli sui dubbi e di fixare a valle ciò che manca: *"inutile arrabbattare la chiusura finché non faccio questa delivery finale… poi se c'è qualcosa che ci siamo persi te lo fixo io"*. Coerente con il workflow deciso in call interna 20/5 (deadline 27/5 → poi ultima demo su stima Federico).
- **Lamentela operativa (pattern)**: bug report scritti male da chi non ha colto i passaggi → spreco di tempo a interpretarli. *"se mi arriva una mail così non ci capisco niente… è scritta male, non è scritta da una persona che ha capito i passaggi"*. Richiesta implicita: filtrare/strutturare le segnalazioni prima di girargliele.

### 2026-05-22 — Risposta Lorena su video tutorial (thread "R: Crowd | Sessione Finale Test")
- **21/5 17:22 — Lorena Cavecchi** si aggancia alla mail Arianna del 19/5 confermando l'esigenza del **video tutorial** promesso: *"effettivamente ci aiuterebbe ad accelerare anche il processo di prova. Quando abbiamo fatto ultimo test, io che ho saltato delle call perché ero via ci ho messo un po' da sola a tentativi a capire"*. Aggiunge richiesta futura: *"quando chiuso tutto potrebbe anche essere utile rifare una call di formazione anche alle altre persone di reparto"*.
- **22/5 — Carlo ha ri-pushato Federico+Simone** sulla produzione del tutorial (era già in lista task #p1 Federico "guida utente Crowd", in piano post-27/5 come "fiocchetto finale" secondo decisione call interna 13/5 — Lorena chiede di anticipare).
- **22/5 — Carlo risponde a Lorena** confermando il push interno fatto e impegnandosi ad aggiornarla appena ha riscontro sui tempi; cenno positivo aperto sulla call di formazione al reparto da pianificare a chiusura progetto.
...
- **Next action Carlo**: email recap a David+Lorena+Arianna (CC Federico+Simone) con punti operativi + setup IT — bozza in [[50 - Archive/Inbox/2026-05/2026-05-12 - BOZZA Email Recap Crowd Sessione Finale]]. Carlo si allinea separatamente con Simone su IT setup prima dell'invio finale o nello stesso thread come punto aperto.

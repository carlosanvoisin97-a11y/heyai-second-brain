...
- **Step 4 — Cattura Code (gap #5) 🟡 DIAGNOSI fatta, recovery+hardening DA FARE**: trovate **16 sessioni vault grosse (2-6/6) mai indicizzate** (no recap/log) — root cause: indexer **finestra-24h + best-effort**. Worklist + piano completo in [[2026-06-07 - Step 4 — Backlog sessioni Code non indicizzate]]. Da decidere (Carlo non ha ancora risposto): profondità recovery (subagent paralleli sui 16 JSONL) + hardening (catch-up mode indexer + hook on-session-end). I JSONL grezzi sono salvi sul Mac (solo locale).

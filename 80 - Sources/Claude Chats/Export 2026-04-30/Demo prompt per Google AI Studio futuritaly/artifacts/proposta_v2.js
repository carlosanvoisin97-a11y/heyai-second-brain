...
function para(text, opts = {}) {
  return new Paragraph({ spacing: { after: opts.after || 120 }, children: [new TextRun({ text, font: "Calibri", size: 22, color: NAVY, ...opts })] });
}

function richPara(runs, after = 120) {
  return new Paragraph({ spacing: { after }, children: runs.map(r => new TextRun({ font: "Calibri", size: 22, color: NAVY, ...r })) });
}

function bullet(text, ref = "bullets") {
  return new Paragraph({ numbering: { reference: ref, level: 0 }, spacing: { after: 60 }, children: [new TextRun({ text, font: "Calibri", size: 22, color: NAVY })] });
}

function richBullet(runs, ref = "bullets") {
  return new Paragraph({ numbering: { reference: ref, level: 0 }, spacing: { after: 60 }, children: runs.map(r => new TextRun({ font: "Calibri", size: 22, color: NAVY, ...r })) });
}

function note(text) {
  return new Paragraph({ spacing: { after: 120 }, children: [new TextRun({ text, font: "Calibri", size: 18, color: GRAY_TEXT, italics: true })] });
}

function spacer() { return new Paragraph({ spacing: { after: 200 }, children: [] }); }
function pageBreak() { return new Paragraph({ children: [new PageBreak()] }); }
function footer(text) {
  return new Paragraph({ spacing: { before: 200, after: 0 }, border: { top: { style: BorderStyle.SINGLE, size: 1, color: TEAL, space: 8 } }, children: [new TextRun({ text, font: "Calibri", size: 16, color: GRAY_TEXT })] });
}
...
const doc = new Document({
  styles: {
    ...
    paragraphStyles: [
      { id: "Heading1", name: "Heading 1", basedOn: "Normal", next: "Normal", quickFormat: true, run: { size: 32, bold: true, font: "Calibri", color: TEAL }, paragraph: { spacing: { before: 360, after: 200 }, outlineLevel: 0 } },
      { id: "Heading2", name: "Heading 2", basedOn: "Normal", next: "Normal", quickFormat: true, run: { size: 26, bold: true, font: "Calibri", color: NAVY }, paragraph: { spacing: { before: 280, after: 160 }, outlineLevel: 1 } },
    ]
  },
  numbering: { config: [
  ...
  sections: [{
    ...
    children: [
      ...
      // TITLE
      new Paragraph({ spacing: { after: 80 }, children: [new TextRun({ text: "Proposta di Investimento", font: "Calibri", size: 36, bold: true, color: TEAL })] }),
      new Paragraph({ spacing: { after: 40 }, children: [new TextRun({ text: "Osservatorio Strategico AI", font: "Calibri", size: 30, color: NAVY })] }),
      new Paragraph({ spacing: { after: 300 }, border: { bottom: { style: BorderStyle.SINGLE, size: 4, color: TEAL, space: 8 } }, children: [new TextRun({ text: "Aggiornamento Marzo 2026", font: "Calibri", size: 20, color: GRAY_TEXT, italics: true })] }),

      // 1. OGGETTO
      heading1("1. Oggetto della Proposta"),
      ...
      new Table({ width: { size: 9506, type: WidthType.DXA }, columnWidths: [2800, 6706], rows: [
      ...
      new Table({ width: { size: 9506, type: WidthType.DXA }, columnWidths: [3000, 6506], rows: [
        ...
        new TableRow({ children: [
          ...
          cell("LLM che analizza il brief cliente e il testo completo di ogni bando, applica regole personalizzate del Cliente e genera uno scoring composito di applicabilit\u00E0 e coerenza (0-100%) con riassunto operativo.", 6506)
        ]}),
        new TableRow({ children: [
          bCell("Parametri di Scoring", 3000, true),
      ...
      new Table({ width: { size: 9506, type: WidthType.DXA }, columnWidths: [2400, 1800, 3306, 2000], rows: [
      ...
      new Table({ width: { size: 9506, type: WidthType.DXA }, columnWidths: [2800, 1800, 4906], rows: [
      ...
      // 6. SICUREZZA E CONFORMITA'
      heading1("6. Sicurezza e Conformit\u00E0"),
      para("L\u2019Osservatorio Strategico AI \u00E8 progettato nel rispetto dei principi di Privacy by Design e by Default (GDPR Art. 25) e delle migliori pratiche di cybersicurezza applicabili a soluzioni cloud-based."),

      heading2("6.1 Principi di Sicurezza Applicati"),
      new Table({ width: { size: 9506, type: WidthType.DXA }, columnWidths: [3200, 6306], rows: [
        new TableRow({ children: [hCell("Principio", 3200), hCell("Implementazione", 6306)] }),
        new TableRow({ children: [bCell("Crittografia", 3200), cell("Tutti i dati sono crittografati in transito (TLS 1.2+) e a riposo (AES-256). Le comunicazioni tra il sistema e le API delle fonti dati avvengono esclusivamente su canali sicuri.", 6306)] }),
        new TableRow({ children: [bCell("Controllo degli accessi", 3200, true), cell("Autenticazione sicura con possibilit\u00E0 di attivazione MFA (Multi-Factor Authentication). Controllo degli accessi basato su ruoli (RBAC) per differenziare i permessi tra utenti operativi e amministratori.", 6306, true)] }),
        new TableRow({ children: [bCell("Localizzazione dei dati", 3200), cell("Infrastruttura cloud ospitata su data center situati nell\u2019Unione Europea, in conformit\u00E0 con i requisiti GDPR in materia di trasferimento dati.", 6306)] }),
        new TableRow({ children: [bCell("Minimizzazione dei dati", 3200, true), cell("Il sistema raccoglie e conserva esclusivamente i dati strettamente necessari al funzionamento (profili aziendali, parametri di ricerca, risultati). Nessun dato personale sensibile viene trattato n\u00E9 inviato ai modelli AI.", 6306, true)] }),
        new TableRow({ children: [bCell("Gestione sicura delle credenziali", 3200), cell("Le chiavi API per l\u2019accesso alle fonti dati (ANAC/PDND, TED, EU F&T) sono gestite tramite sistemi di secret management e non sono mai esposte nel codice sorgente o nei log.", 6306)] }),
        new TableRow({ children: [bCell("Logging e audit trail", 3200, true), cell("Tutte le operazioni rilevanti (accessi, ricerche, modifiche configurazione) sono registrate in log immutabili per garantire tracciabilit\u00E0 e accountability.", 6306, true)] }),
        new TableRow({ children: [bCell("Backup e disaster recovery", 3200), cell("Backup automatici periodici dei dati e delle configurazioni. Piano di ripristino definito per garantire la continuit\u00E0 operativa in caso di incidenti.", 6306)] }),
        new TableRow({ children: [bCell("Aggiornamenti di sicurezza", 3200, true), cell("Applicazione regolare di patch di sicurezza alle componenti software e infrastrutturali, con monitoraggio delle vulnerabilit\u00E0 note (CVE).", 6306, true)] }),
      ]}),

      heading2("6.2 Conformit\u00E0 Normativa"),
      bullet("GDPR (Regolamento UE 2016/679): il sistema \u00E8 progettato per garantire la conformit\u00E0 ai principi di liceit\u00E0, trasparenza, minimizzazione, limitazione della conservazione e integrit\u00E0 dei dati."),
      bullet("Linee guida ACN (Agenzia per la Cybersicurezza Nazionale): l\u2019architettura segue le indicazioni per lo sviluppo di software sicuro e per l\u2019erogazione di servizi cloud."),
      bullet("Standard di riferimento: le pratiche di sicurezza adottate sono allineate ai framework ISO 27001 (gestione sicurezza informazioni) e OWASP (sicurezza applicativa web)."),
      note("Nota: in caso di attivazione delle funzionalit\u00E0 evolutive che prevedono il trattamento di dati personali (es. profili clienti con dati di contatto per notifiche proattive), sar\u00E0 necessario predisporre idonea documentazione privacy (informativa, registro trattamenti, eventuale DPIA) in collaborazione con il Cliente."),

      pageBreak(),

      // 7. INVESTIMENTO
      heading1("7. Investimento"),
      ...
      spacer(),
      heading2("7.1 MVP \u2013 Investimento Base"),
      new Table({ width: { size: 9506, type: WidthType.DXA }, columnWidths: [6506, 3000], rows: [
        new TableRow({ children: [hCell("Voce", 6506), hCell("Importo", 3000)] }),
        new TableRow({ children: [cell("Sviluppo, configurazione e personalizzazione piattaforma", 6506), cell("\u20AC 26.000", 3000)] }),
      ...
      spacer(),
      heading2("7.2 Funzionalit\u00E0 Evolutive \u2013 Add-on quotati separatamente"),
      ...
      new Table({ width: { size: 9506, type: WidthType.DXA }, columnWidths: [4000, 3506, 2000], rows: [
        new TableRow({ children: [hCell("Funzionalit\u00E0", 4000), hCell("Descrizione", 3506), hCell("Stima", 2000)] }),
        new TableRow({ children: [
          bCell("Notifiche Proattive", 4000),
          cell("Monitoraggio attivo con incrocio quotidiano dei profili clienti e delle nuove opportunit\u00E0. Report giornaliero con azioni pendenti e motivazione per ogni opportunit\u00E0.", 3506),
          cell("\u20AC 3.000 \u2013 5.000", 2000)
        ]}),
        new TableRow({ children: [
          bCell("Osservatorio Preventivo", 4000, true),
          cell("Anticipazione delle opportunit\u00E0 a partire da direttive normative (legge di bilancio, decreti attuativi, programmazioni europee) prima della pubblicazione dei bandi.", 3506, true),
          cell("\u20AC 3.000 \u2013 5.000", 2000, true)
        ]}),
        new TableRow({ children: [
          bCell("Generazione Email e Presentazioni", 4000),
          cell("Creazione automatica di messaggi e presentazioni personalizzate per agganciare il cliente sull\u2019opportunit\u00E0 identificata, con integrazione ambiente email.", 3506),
          cell("\u20AC 3.000 \u2013 5.000", 2000)
        ]}),
      ]}),
      ...
      spacer(),
      heading2("7.3 Incluso nell\u2019investimento MVP"),
      ...
      bullet("Input da brief cliente con ricerca automatica incrociata su tutte le fonti"),
      bullet("Scoring composito a 6 dimensioni con pesi configurabili e breakdown visuale"),
      ...
      // 8. PAGAMENTO
      heading1("8. Modalit\u00E0 di Pagamento"),
      ...
      // 9. VANTAGGI
      heading1("9. Vantaggi Competitivi"),
      new Table({ width: { size: 9506, type: WidthType.DXA }, columnWidths: [3200, 6306], rows: [
        ...
        new TableRow({ children: [bCell("Sicurezza by design", 3200, true), cell("Architettura conforme GDPR, crittografia end-to-end, dati in UE, nessun dato sensibile esposto ai modelli AI.", 6306, true)] }),
      ]}),
      spacer(),

      // 10. CONDIZIONI
      heading1("10. Condizioni Generali"),
      new Table({ width: { size: 9506, type: WidthType.DXA }, columnWidths: [3200, 6306], rows: [
        ...
        new TableRow({ children: [bCell("Garanzia", 3200, true), cell("2 mesi assistenza post-lancio (bug fixing e supporto, fino a 20 ore)", 6306, true)] }),
      ...
      // 11. REVENUE SHARE
      heading1("11. Modello di Collaborazione \u2013 Revenue Share"),
      richPara([
      ...
      heading2("11.2 Approccio in tre fasi"),
      richBullet([{ text: "Fase 1 \u2014 MVP: ", bold: true }, { text: "Sviluppo dello strumento a costo agevolato rispetto al prezzo standard, con licenza d\u2019uso completa per FuturItaly. HeyAI assorbe come investimento proprio i costi di PM, analisi funzionale, coordinamento, change management e formazione." }]),
      ...
      heading2("11.3 Ipotesi illustrativa di Revenue Share"),
      ...
      heading2("11.4 Elementi da definire congiuntamente"),
      ...
      bullet("Entit\u00E0 dello sconto sull\u2019investimento iniziale in funzione dei termini di partnership"),
      ...
      spacer(),
      spacer(),
      footer("HeyAI S.r.l. | PEC: heyai@pec.it | REA RM \u2013 1751505 | Cod. Fiscale: 17947791004 | Amministratore Unico: Ing. Bruno Mattucci"),
    ]
  }]
});

Packer.toBuffer(doc).then(buffer => {
  fs.writeFileSync("/home/claude/proposta_v2.docx", buffer);
  console.log("Done!");
});
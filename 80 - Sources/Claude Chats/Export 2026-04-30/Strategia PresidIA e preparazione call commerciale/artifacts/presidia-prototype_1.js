...
const COMPANY = {
  ...
  coperti: "—", superficieFascia: "50 – 100 mq", postazioni: "—",
};
...
const Val = ({children,style}) => <span style={{fontSize:13.5,color:C.text,...style}}>{children}</span>;
...
const InfoRow = ({label,value,style}) => (
  <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",padding:"10px 0",borderBottom:`1px solid ${C.border}`,...style}}>
...
const NavItem = ({icon,label,active,onClick,badge:b,disabled}) => (
  <div onClick={disabled?undefined:onClick} style={{display:"flex",alignItems:"center",gap:11,padding:"9px 14px",borderRadius:7,cursor:disabled?"default":"pointer",transition:"all 0.15s",background:active?C.accentGlow:"transparent",color:active?C.accent:disabled?C.textDim:C.textMuted,fontWeight:active?600:400,fontSize:13.5,opacity:disabled?0.5:1,position:"relative"}}>
  ...
  <div>
    ...
    <Row>
      <Card style={{flex:1,minWidth:300}}>
        ...
        <InfoRow label="Fascia fatturato" value={COMPANY.fatturato} style={{borderBottom:"none"}}/>
      </Card>
      <Card style={{flex:1,minWidth:300}}>
        ...
        <InfoRow label="N. dipendenti attivi" value={COMPANY.dipendenti} style={{borderBottom:"none"}}/>
      </Card>
    </Row>
    <Row style={{marginTop:12}}>
      <Card style={{flex:1,minWidth:300}}>
        ...
        <InfoRow label="Email" value={COMPANY.emailConsulente} style={{borderBottom:"none"}}/>
      </Card>
      <Card style={{flex:1,minWidth:300}}>
        ...
        <InfoRow label="Tipologia" value={COMPANY.rsppTipo} style={{borderBottom:"none"}}/>
      </Card>
    </Row>
...
const PCedolini = () => (
  <div>
    ...
    <Card style={{padding:0,overflow:"hidden"}}>
      ...
      <Table cols={[
        {label:"Mese",key:"mese"},
        {label:"File",key:"file",render:r=><span style={{fontFamily:MONO,fontSize:12,color:C.accent}}>{r.file}</span>},
        {label:"Cedolini",key:"pagine"},
        {label:"Stato",render:r=><Badge s="green" label={r.stato}/>},
        {label:"Data elaborazione",key:"data",render:r=><span style={{fontFamily:MONO,fontSize:12}}>{r.data}</span>},
      ]} data={CEDOLINI_LOG}/>
    </Card>
...
const PDash = ({go}) => {
  ...
  return <div>
    ...
    <Row style={{marginBottom:16}}>
      {[{n:rc,c:C.red,l:"Criticità",s:"Richiede attenzione immediata",gl:C.redBg},{n:yc,c:C.yellow,l:"Monitoraggio",s:"Elementi da osservare",gl:C.yellowBg},{n:4,c:C.green,l:"Aree stabili",s:"Situazione regolare",gl:C.greenBg}].map((x,i)=>
        <Card key={i} onClick={i<2?()=>go("notifiche"):undefined} glow={i<2?x.c+"15":undefined} style={{flex:1,minWidth:180,borderLeft:`3px solid ${x.c}`}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
...
const PPersonale = () => <div>
  ...
  <Card style={{padding:0,overflow:"hidden"}}>
    <Table cols={[
      ...
      {label:"ID",key:"id",render:r=><span style={{fontFamily:MONO,fontSize:12}}>{r.id}</span>},
  ...
  return <div>
    ...
    <Card style={{padding:0,overflow:"hidden",marginBottom:14}}>
      <Table cols={[
        ...
        {label:"Straordinari (h)",render:r=>r.strao>15?<span style={{color:C.red,fontWeight:600}}>{r.strao}</span>:r.strao},
      ]} data={EMP}/>
    </Card>
    <Card>
...
const PQA = () => {
  ...
  const runFlow = useCallback(()=>{
    if(!started){setStarted(true);setStep(0);setMsgs([]);runStep(0);}
  },[started]);

  const runStep = (s) => {
    if(s>=QA_FLOW.length) return;
    const item=QA_FLOW[s];
    setTimeout(()=>{
      setMsgs(p=>[...p,{role:"user",text:item.q}]);
      setTyping(true);
      setTimeout(()=>{
        setTyping(false);
        setMsgs(p=>[...p,{role:"ai",text:item.a}]);
        setStep(s+1);
        setTimeout(()=>runStep(s+1), 2500);
      }, 2000);
    }, s===0?500:2500);
  };
  ...
  return <div>
    ...
    <Card style={{padding:0,overflow:"hidden",minHeight:420,display:"flex",flexDirection:"column"}}>
      ...
      <div ref={chatRef} style={{flex:1,overflowY:"auto",padding:"18px",display:"flex",flexDirection:"column",gap:14}}>
        {!started && <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:14}}>
          ...
          <div style={{fontSize:14,color:C.textMuted,textAlign:"center",maxWidth:320,lineHeight:1.6}}>L'assistente normativo risponde a domande sul CCNL Commercio con riferimenti testuali agli articoli.</div>
          <div onClick={runFlow} style={{padding:"10px 24px",borderRadius:7,background:C.accent,color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer",marginTop:6}}>
            ▶ Avvia demo Q&A
          </div>
        </div>}
        {msgs.map((m,i)=>(
        ...
        {typing && <div style={{display:"flex",justifyContent:"flex-start"}}>
          <div style={{padding:"12px 16px",borderRadius:"12px 12px 12px 2px",background:C.surfaceAlt}}>
            <span style={{color:C.textMuted,fontSize:13}}>
              <span style={{animation:"pulse 1.2s ease infinite"}}>Analisi normativa in corso...</span>
            </span>
          </div>
        </div>}
      </div>
...
export default function App() {
  ...
  return (
    <div style={{fontFamily:FONT,background:C.bg,color:C.text,minHeight:"100vh",display:"flex",opacity:ready?1:0,transition:"opacity 0.4s"}}>
      ...
      <style>{`*{box-sizing:border-box;margin:0;padding:0}::-webkit-scrollbar{width:5px}::-webkit-scrollbar-track{background:transparent}::-webkit-scrollbar-thumb{background:${C.borderLight};border-radius:3px}@keyframes pulse{0%,100%{opacity:1}50%{opacity:.35}}`}</style>

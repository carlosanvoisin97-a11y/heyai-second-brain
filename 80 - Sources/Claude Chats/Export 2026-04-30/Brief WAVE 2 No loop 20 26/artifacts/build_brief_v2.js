...
// Box di nota neutro (per evidenziare punti chiave senza "allarme")
...
children.push(new Paragraph({
  children: [new TextRun({ text: "Versione 2.0", italics: true, size: 22, color: C.gray, font: "Calibri" })],
  alignment: AlignmentType.CENTER,
  spacing: { after: 600 }
}));
...
children.push(new Table({
  ...
  rows: [
    ...
    new TableRow({ children: [
      ...
      dCell("Evoluzione del modulo di trascrizione e sintesi automatica delle riunioni, con estensione alla cattura audio per call esterne (priorità alta).", modW[1], { fill: C.alt }),
...
// ============================================================
// 4. PIANO DI FATTURAZIONE E MILESTONE
// ============================================================
...
children.push(p("Per ciascun modulo è previsto un periodo di manutenzione e garanzia di 3 (tre) mesi, decorrenti dalla data di consegna finale del modulo medesimo, in coerenza con quanto pattuito nel contratto Wave 1."));
...
children.push(p("Le modalità operative di apertura ticket, classificazione di severità e tempi di risposta seguono il regime già disciplinato nel contratto Wave 1, salvo diversa pattuizione."));
...
// ============================================================
// 8. ACCETTAZIONE DEI DELIVERABLE
// ============================================================
...
children.push(p("L'articolato GDPR del contratto Wave 1 va integrato per coprire il perimetro sopra descritto, con particolare riguardo a: (i) trattamento di dati di terzi (partecipanti agli eventi, contatti commerciali esterni); (ii) uso di provider di Large Language Model esterni (OpenAI / Anthropic / altri), i cui costi di consumo restano a carico del Committente; (iii) gestione dei pagamenti dei partecipanti tramite gateway terzo, con valutazione degli adempimenti PSD2 ove applicabili."));

// ============================================================
// 10. LIMITAZIONI DI RESPONSABILITÀ E DISCLAIMER
// ============================================================
...
// ============================================================
// 11. NOTE TECNICHE PER LA STESURA
// ============================================================
...
children.push(noteBox(
  ...
  [
    "La proposta commerciale consegnata al Committente (\"Roadmap di Sviluppo Ecosistema AI Noloop 2026\") contiene business case quantitativi (stime di risparmio operativo, payback, ore liberate) e riferimenti alle agevolazioni fiscali italiane (Iperammortamento ex L. 199/2025).",
...
children.push(noteBox(
  "Efficacia retroattiva limitata",
  [
    "Alcuni moduli (Venue Finder Evolution e Minutes Extension) sono in fase di sviluppo avviato precedentemente alla firma del contratto, in continuità con le attività della Wave 1. Si chiede al legale di prevedere, in coerenza con quanto già adottato nel contratto Wave 1, una clausola di efficacia retroattiva limitatamente a tali sviluppi, con espressa ratifica delle attività eseguite."
  ]
));

// ============================================================
// 12. ALLEGATI E DOCUMENTI DI RIFERIMENTO
// ============================================================
...
children.push(bullet([
  ...
  new TextRun({ text: "(da utilizzare come modello strutturale di riferimento — 18 articoli);", font: "Calibri", size: 22 })
]));
children.push(bullet([
  new TextRun({ text: "Allegato Tecnico Wave 1 ", bold: true, font: "Calibri", size: 22 }),
  new TextRun({ text: "(per il formato dell'allegato tecnico al nuovo contratto);", font: "Calibri", size: 22 })
]));
children.push(bullet([
...
children.push(spacer(120));
children.push(p("La \"Roadmap di Sviluppo Ecosistema AI Noloop 2026\" (proposta commerciale consegnata al Committente) non costituisce parte integrante del contratto e non è quindi allegata. Resta a disposizione del legale a fini di consultazione, su richiesta.", { italics: true, color: C.gray, size: 20 }));

// ============================================================
// 13. CHECKLIST OPERATIVA PER IL LEGALE
// ============================================================
children.push(h1("13. Checklist operativa per la stesura"));

children.push(p("Sintesi delle indicazioni operative contenute nel presente brief:"));

children.push(bullet("Mantenere la struttura del contratto Wave 1 (18 articoli) come ossatura."));
children.push(bullet("Riscrivere ex novo: oggetto (§3), corrispettivo (§5), piano di pagamento (con la tabella mensile di §4 del presente brief), articolato IP (§5 del brief — divergenza sostanziale dal Wave 1), articolato GDPR integrato, manutenzione (perimetro 3 mesi confermato)."));
children.push(bullet("Aggiungere ex novo: clausola change request (§7), procedura accettazione deliverable con silenzio-assenso a 15 giorni (§8), articolato disclaimer e limitazioni di responsabilità (§10), clausola di sospensione bilaterale agosto, clausola esclusione benefici economici e fiscali."));
children.push(bullet("Risolvere con il Committente la valutazione tecnica della clausola sulla trasferibilità della licenza in caso di operazioni straordinarie (§5.2)."));
children.push(bullet("Richiamare espressamente il Requirement Log come parte integrante e sostanziale del contratto, gestendo i raccordi di nomenclatura come da nota al §11."));
...
const doc = new Document({
  creator: "HeyAI S.r.l.",
  title: "Brief per Stesura Contratto — Noloop Wave 2 (v2)",
  styles: {
...
Packer.toBuffer(doc).then(buffer => {
  fs.writeFileSync("/home/claude/Brief_Avvocato_Wave2_Noloop_v2.docx", buffer);
  console.log("OK");
});
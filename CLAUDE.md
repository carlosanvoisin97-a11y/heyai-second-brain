...
- **Strategia & PM**: deliverable cliente/quotazione/prep call/audit pre-consegna → `pm-method` (il "modo Carlo": checklist a fasi + 8 regole invarianti — orchestratore) · revisione slide/deck di gara → `deck-iteration` · stress-test numeri pre-consegna → `pricing-audit` · post-mortem performance call → `coaching-note` · requirement log / analisi requisiti cliente → `requirement-log` · analisi/decisione/matrice/memo MECE → `management-consulting` (ora con matrice fornitore HeyAI) · brainstorm idea → `brainstorming` / `product-management:product-brainstorming` · spec-PRD → `product-management:write-spec` · competitive brief/battlecard → `product-management:competitive-brief` / `marketing-skills:competitor-profiling` · sprint/roadmap/metrics/stakeholder update → `product-management:*` · sintesi ricerca utente → `design:research-synthesis`
...
- **Sistema / vault / meta**: salute vault (wikilink/MOC/consistenza/contraddizioni) → `vault-health` (sub-check links/moc/consistency/contradictions, default all) · trascrizione call → `transcript-processing` · vault-live commit non banale → `vault-live-protocol` · memoria (persone/progetti/shorthand) → `productivity:memory-management`/`consolidate-memory` · config/hook/permessi → `update-config` · loop/scheduled agents → `loop`/`schedule` · review/security → `verify`/`code-review`/`security-review` · creare skill/connettore MCP/plugin Cowork → `skill-creator`/`mcp-builder`/`cowork-plugin-management:*`
...
- **Lavora sempre su `main`** in `/Users/carlosanvoisin/claude` (= cartella Obsidian).
- **Niente scritture concorrenti sullo stesso artefatto (regola 10/6/2026)**: una sola sessione/agente alla volta scrive su un dato file; le sessioni parallele lavorano su file disgiunti o in sola lettura. Ricorda che il **plugin Obsidian Git è un terzo writer sempre attivo** (commit-and-sync ~10' ad app aperta): pusha anche i commit locali altrui — pull-before-write ad ogni commit, e per i lavori delicati valutare Obsidian chiuso. La worktree per-sessione (`.claude/worktrees/...`) è creata in automatico dall'harness: il lavoro VERO deve atterrare su `main`, non restare in worktree.
...
**Gerarchia fonte-di-verità (regola 10/6/2026, lesson fork PPTX da sorgente sbagliata)**: prima di produrre o modificare un deliverable, DICHIARARE in chat quale artefatto è la fonte di verità e in che ordine vincono i conflitti (tipicamente: brief cliente > Master Facts Sheet > scheda progetto > versione live/file precedente). Ogni edit parte SOLO dalla fonte dichiarata — mai fork da una copia non canonica.
...
- **Contratto di stabilità ancore (10/6/2026)**: i numeri di sezione **§9, §13, §15** (con sotto-sezioni bis/ter/quater/quinquies) sono citati dai prompt delle routine cloud, dagli scheduled task e dalle skill — **non rinumerarli mai**. Nuove sezioni si aggiungono con suffissi (es. §9quater), non spostando le esistenti.
